...
function renderConsent() {
  ...
  app.innerHTML = `
    <section class="intro-screen">
      ...
      <h1>Bank app interaction study</h1>
      <p class="lead">This short banking-style prototype records interaction patterns from a guided demo session.</p>
      <div class="privacy-grid">
        ...
        <article class="info-card accent-teal"><span class="card-icon">Safe</span><h2>Not collected</h2><p>No user-entered text content, no location, no real banking data and no manual personal identifier.</p></article>
        <article class="info-card accent-yellow"><span class="card-icon">Flow</span><h2>What you do</h2><p>Complete a guided bank app review: search, scroll, categorise, move money, review insights and reply securely.</p></article>
      </div>
...
function renderBankShell() {
  ...
  app.innerHTML = `<section class="bank-screen"><header class="bank-header"><div><p class="eyebrow">Bank app</p><h1>${headingForArea(state.activeArea)}</h1></div><button class="avatar" data-component-id="profile_button">${escapeHtml(state.identity?.participantId?.slice(1,3) || "ID")}</button></header><div class="review-banner"><div><p>${task.id === "finish_feeling" ? "Final check" : `Review step ${state.currentTaskIndex + 1} of ${GUIDED_TASKS.length}`}</p><strong>${escapeHtml(task.title)}</strong><span>${instructionForTask(task)}</span></div><div class="progress-ring">${Math.round(((state.currentTaskIndex + 1) / GUIDED_TASKS.length) * 100)}%</div></div><div class="bank-content">${renderAreaContent(task)}</div><div id="taskWarning" class="task-warning" hidden></div>${task.type === "finish" ? "" : `<button class="primary-btn sticky-action" data-component-id="continue_button" id="continueBtn">Continue</button>`}${renderBottomNav()}</section>`;
  attachTaskElementLogging(document);
...
function headingForArea(area) { return { home: "Home", activity: "Activity", pots: "Pots", insights: "Insights", secure: "Secure" }[area] || "Bank app"; }
function instructionForTask(task) {
  const target = state.content.target;
  const map = { home_balance_check: "Tap the Current Account card after checking the balance.", activity_search: `Search for: ${target.merchant}`, activity_scroll_select: `Find ${target.merchant} from last month and tap it.`, transaction_category: `Categorise ${target.merchant} as ${target.category}.`, transaction_note: `Copy this note: ${target.note}`, pots_transfer: `Move GBP ${state.content.transferAmount} to Travel Pot.`, insights_review: `Tap the insight: ${state.content.insightTarget.title}.`, secure_approval: `Approve ${state.content.approvalPayment.merchant} for ${state.content.approvalPayment.amount}.`, secure_reply: `Copy: ${target.reply}`, finish_feeling: "Choose how this review felt." };
...
function renderHome() { return `<div class="balance-hero"><span>Bank app</span><p>Good afternoon</p><h2>GBP 2,112.77</h2><small>Across demo accounts</small></div><div class="card-list">${state.content.accounts.map((a) => `<button class="account-card ${a.accent}" data-account-id="${a.id}" data-component-id="account_${a.id}"><span><strong>${escapeHtml(a.name)}</strong><small>${escapeHtml(a.sub)}</small></span><b>${escapeHtml(a.balance)}</b></button>`).join("")}</div><div class="mini-grid"><div class="mini-card"><span>Pots</span><strong>3 pots</strong><small>GBP 1,556 saved</small></div><div class="mini-card"><span>Alerts</span><strong>2 alerts</strong><small>Ready to review</small></div></div>`; }
function renderActivitySearch() { return `<div class="section-card search-card"><p class="muted">Find this merchant</p><h2>${escapeHtml(state.content.searchTerm)}</h2><label class="input-label" for="searchInput">Search transactions</label><input id="searchInput" data-component-id="search_input" class="text-input" autocomplete="off" autocapitalize="none" spellcheck="false" placeholder="Type merchant name" /></div>${renderRecentPreview()}`; }
...
function bindTypingTask(task, inputOverride = null, targetOverride = null) {
  ...
  input.addEventListener("input", () => {
    ...
    const corrections = (getEvidence(task.id).corrections || 0) + (delta < 0 ? 1 : 0);
    previousLength = currentLength;
    const normalisedMatch = normaliseForValidation(input.value) === normaliseForValidation(target);
    updateEvidence(task.id, { inputLength: currentLength, targetLength: target.length, deltaLengthLast: delta, corrections, completedLength: currentLength >= target.length, exactMatch: normalisedMatch, exactCaseSensitiveMatch: input.value === target, validationMode: "case_insensitive_trimmed" });
  });
}

function expectedTextForTask(task) { if (task.id === "activity_search") return state.content.searchTerm; if (task.id === "transaction_note") return state.content.target.note; if (task.id === "secure_reply") return state.content.target.reply; if (task.id === "pots_transfer") return state.content.transferAmount; return ""; }
...
function validateTask(task) { const ev = getEvidence(task.id); switch (task.id) { case "home_balance_check": return ev.accountReviewed ? null : "Tap the Current Account card to continue."; case "activity_search": return ev.exactMatch ? null : `Type the merchant: ${state.content.searchTerm}`; case "activity_scroll_select": return ev.selectedTarget ? null : `Tap the ${state.content.target.merchant} payment from last month.`; case "transaction_category": return ev.correctCategory ? null : `Choose ${state.content.target.category}.`; case "transaction_note": return ev.exactMatch ? null : `Copy the note: ${state.content.target.note}`; case "pots_transfer": return ev.travelPotSelected && ev.exactMatch && ev.moveConfirmed ? null : `Select Travel Pot, type ${state.content.transferAmount}, then tap Move money.`; case "insights_review": return ev.targetInsightTapped ? null : `Tap the ${state.content.insightTarget.title} insight.`; case "secure_approval": return ev.correctAction ? null : `Choose ${state.content.expectedApprovalAction}.`; case "secure_reply": return ev.exactMatch ? null : "Copy the secure reply."; default: return null; } }
